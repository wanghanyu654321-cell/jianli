...
|---|---|---|---|---|---|---|---|
| FCT-001 | 简历母版.docx | 本地源文件（未上传） | FACT | 2026-09-12 22:21 local file time; core property modified 2026-09-12 14:21 UTC | Highest | 候选人事实、责任边界、数字口径、项目状态 | 标题为“求职简历事实母版 V3”；唯一事实母版。 |
...
| DER-001 | 简历母版.txt | `work\source-extracted\简历母版.txt` | SUPPORT | 2026-09-13 generated | Derived | DOCX 段落抽取，便于行号核验 | 从 FCT-001 派生，不能取代原 DOCX。 |
...
事实源在 `简历母版.txt:381` 引用 `wanghanyu654321-cell/-agent` 与 `job-ready/integration-v1`。本次仅在用户确认的当前本地资料目录和当前 2026-09-13 工作区内查找，未发现对应 checkout；同目录现有仓库 README 明确是 Seedance 集成项目，不能冒充客服 Agent 仓库。因此未调用 `repo-to-resume`，也未把 README、历史工作区或互联网内容写入事实账本。

## Source resolution result

两份母版均有明确 V3 标题；文件名分别唯一；正文与修改时间没有冲突。来源解析通过，可以继续建立 Claim Ledger、Requirement × Evidence Matrix 和岗位族基线。当前没有单条公司/岗位 JD，因此不得生成 Single-JD Tailored Resume、ATS 最终审查或投递版 DOCX/PDF。
