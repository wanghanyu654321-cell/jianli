...
7. **Bullet 标题只负责定位 JD 关键词，不负责把整句内容再概括一遍。** 优先使用“POC 交付、RAG 验证、权限控制、系统集成、回归测试、上线指标”等短标题；避免“真实门店 POC 与使用、工程验收与客户价值边界、真实使用后的工程验证”这类长标题。
8. “真实”只在需要区分 Demo / 模拟环境时使用一次；同一段内已经明确 POC / 实际使用后，不重复堆叠“真实门店、真实渠道、真实 Workflow、真实使用”。
