...
- **工具调用 / 权限控制：** 模型负责理解用户意图并提出动作，但 身份、成员关系、能力权限、业务范围与最终写权限由服务端决定，避免 Prompt 或模型输出直接获得业务授权。
- **服务边界设计：** Python / FastAPI 被限制为 内部候选证据服务，只负责返回候选证据，不负责批准知识、扩大业务范围 或改变业务状态；最终授权保留在掌握身份、业务上下文和持久化状态的 Node 主应用侧，避免形成多个权威中心。
- **业务状态与持久化：** Ticket / 人工接管 等业务动作只有完成 **权限检查 → 写入 → 持久化 → 按权限范围回读** 后才确认成功，解决模型文本中的“已完成”与真实系统状态不一致的问题。
...
- **RAG 集成与质量边界：** pgvector / FastAPI / Node cross-language integration PASS 只能证明数据和 contract 链路成立，不能证明真实 Query 的 semantic retrieval quality 已通过；Hosted Embedding 和 真实业务场景下的检索质量 acceptance 因此保持独立状态，不用“系统打通”替代“效果已经好”。
- **Badcase 根因分析：** Agent 出错后先判断知识库 / 证据、Prompt、路由、工具、权限还是业务流程，再修改对应层，避免长期使用 Prompt 覆盖底层问题。
- **回归验证与质量门槛：** 冻结测试集、负向测试用例、留出测试集、回归测试和业务结果验收共同用于判断 Prompt / RAG / 工具 / 业务流程 修改是否破坏已有行为。
