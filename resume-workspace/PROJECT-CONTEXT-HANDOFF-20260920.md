...
- 朗臻成长路径必须显式保留：**牙膏品类电商运营 → 宠物项目运营管理 → 约 5 个跨平台店铺 / 年度整体 GMV 约 1,500 万**；同时保留优先级、项目排期、阶段检查、跨团队协调、方案 / 进展汇报。
- 用户的“学习 / 适应能力”通过跨品类、跨渠道、跨岗位责任升级和结果体现，不写空泛人格形容词。
- 今宜产品定位表述聚焦“目标用户 + 核心卖点 + 内容验证 / 迁移”，不再以泛化“需求增长”作为标题逻辑。
- 今宜该 Bullet 当前改为“内容策略 / 卖点验证”：目标用户 + 核心卖点 → 短视频 / 直播验证 → 有效卖点与视觉表达迁移；不再写过细的“健身轻食目标用户重新调整玉米包”。
- 朗臻宠物项目的新媒体运营能力必须显式保留：负责小红书策略与运营管理，协同约 5 人团队，建立选题 / 内容测试 / 有效或爆文方向验证 / 复刻 / 数据迭代流程。
...
## 25.1E ONLINE 与角色版的投递逻辑

用户最新明确 ONLINE 与角色版的职责边界：

### ONLINE-MAIN 的目标
ONLINE-MAIN 不是完整 Role Baseline，也不是某一个 JD 的深度定制版。它的目标是：
- 让 HR / 面试官在第一眼先看到最强、最直接的正式 AI 证据：**AI 评测 / Agent 评测**；
- 当招聘方发现候选人具备岗位所需的评测能力后，再通过其余经历快速看到候选人的差异化能力与特点；
- 差异化能力重点包括：**沟通、协调、项目管理、两段 0→1 成功、复杂环境下的问题判断与推进、能力迁移**；
- ONLINE 的最终作用是提高“愿意继续沟通 / 愿意索要正式简历”的概率，而不是在一个页面里穷尽所有岗位证据。

### R1–R6 / JD-specific 的目标
当招聘方进入进一步沟通或需要正式简历后，再根据目标岗位提高对应 Role Evidence 的占比：
- R1：增加 AI / Agent 评测、质量治理、Query / RAG、Bad Case、Path Eval、Regression / Harness 等细节；
- R2：增加需求拆解、方案设计、系统集成、POC、交付与现场问题解决；
- 其他 Role 按各自 JD 提高对应证据密度。

### 不可丢失的通用能力
即使进入 R1 等强角色化版本，也不能因为“岗位更聚焦”而删除以下通用核心：
- 项目管理能力；
- 跨团队沟通 / 协调能力；
- 已取得的项目结果；
- 两段 0→1 成功；
- 面对陌生 / 复杂问题进行拆解、判断、推进和验证的能力；
- 从执行到管理、从传统业务到 AI 场景的能力迁移事实。

因此：
**ONLINE = 强匹配入口 + 差异化能力展示；Role Baseline / JD-specific = 对应岗位证据放大 + 通用能力底座保留。**

禁止再把 ONLINE 写成“所有能力平均分配”，也禁止把 R1 写成“只剩评测、删除管理 / 0→1 / 项目成功能力”。

## 25.1F HR-first 成果提炼规则

用户最终确认的写作逻辑：
- 所有简历内容先站在 HR / 招聘官视角判断，而不是先站在技术人员视角；
- HR 首先依赖岗位词 / 能力词做检索与快速匹配，因此标题必须使用招聘市场可识别词；
- 专业深度由正文承接，不要求 HR 先理解内部术语；
- 不写“学习能力强、适应能力强”这类空泛结论，而应从成果中提炼能力，让招聘方通过事实主动得出结论。

成果 → 能力示例：
- 牙膏运营 → 约 5 个跨平台店铺 / 年度整体 GMV 约 1,500 万：证明责任升级、项目经营和管理能力；
- 抖音新盘月 GMV 约 7 万 → 43 万：证明 0→1、问题判断、数据验证和跨团队推进；
- 小红书约 5 人团队 + 内容测试 / 复刻：证明新媒体运营、方法建立和团队协同；
- Data Agent 一致性约 80%→95%、复杂任务约 60%→85%：证明质量治理、规则转译和问题闭环；
- 微信部署 / 实体门店 POC / 开始使用：证明需求拆解、方案落地、系统集成和验收能力。

ONLINE 与角色版：
- ONLINE = 先让招聘方看到评测直接匹配，再用成果和通用能力制造差异化兴趣；
- R1–R6 = 对应岗位证据加权，但通用能力底座始终保留。
