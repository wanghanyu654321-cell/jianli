...
- **复杂业务规则理解与转译：** 承接上级提供的业务规则和任务要求后，先理解业务目标、数据口径和判断边界，再将复杂规则转化为下游 BPO 可以执行的标准和培训内容，减少上游业务语言与下游执行理解之间的偏差。
- **需求澄清与问题反馈：** 在实际执行前主动识别规则定义不清、Context 不足或场景覆盖缺口，并集中与上级确认；执行过程中持续同步进度、发现的问题和待确认事项，使上级能够及时调整判断标准或提供补充信息。
- **复杂问题解释：** 对反复出现的 Badcase，结合执行人员理解、规则本身和业务 Context 判断问题来源，并推动规则、示例、培训或执行边界调整；无法稳定裁定的争议 Case 则整理核心问题后升级确认。
- **规模与结果：** 单项任务下游 BPO 执行人数 **10+ 人**，每周处理约 **5,000–10,000 条任务**；累计沉淀 **10+ 份 SOP / 规则 / 执行文档**，部分任务一致性约 **80% → 95%**，复杂任务约 **60% → 85%**。
...
- **合作结果：** 达人合作 **单月销售额合计 10 万元+**；同一新盘账号月 GMV 从早期约 **7 万**提升至后续首月约 **43 万**。
